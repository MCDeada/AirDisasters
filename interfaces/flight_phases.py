...
class FlightPhases(Base):
    ...
    def get_rows_all_foreign(self) -> Response:
        ...
        res_dict = {row['id']: row['phase_name'] for row in res['result']}
        return jsonify({'error': False, 'result': res_dict})

    def update(self, data: Dict[str, Any]) -> Response:
        sql = f"""
        UPDATE Flight_phases
        SET phase_name = %s
        WHERE id = %s;
        """
        tupled_data = (
            data['phase_name'], data['row_id'],
        )
